...
def Cosine4AIF_ExtKety_pat(t, aif, ke, dt, ve, vp, device='cpu'):
    aif = torch.t(aif)
    t = t.unsqueeze(0).repeat(len(ke), 1).to(device)
    t = t - aif[0].unsqueeze(1) - dt
    shapes = t.size()
    ...
    aif1 = aif[1].unsqueeze(1).repeat(1, shapes[1]).to(device)
    aif2 = aif[2].unsqueeze(1).repeat(1, shapes[1]).to(device)
    aif3 = aif[3].unsqueeze(1).repeat(1, shapes[1]).to(device)
    aif4 = aif[4].unsqueeze(1).repeat(1, shapes[1]).to(device)
...
def Cosine4AIF_ExtKety_pop(t, aif, ke, dt, ve, vp, device='cpu'):
    # offset time array
    t = t - aif[0] - dt
    ct = torch.zeros(np.shape(t)).to(device)
    shapes = t.size()
...
def Cosine8AIF_ExtKety_pepijn(t, aif, ke, dt, ve, vp, device='cpu'):
    # offset time array
    t = t.unsqueeze(dim=0).repeat(len(ke), 1)
    t = t - aif[0] - dt
    
    shapes = t.size()
    dt = dt.repeat(1, shapes[1])
    ke = ke.repeat(1, shapes[1])
    ve = ve.repeat(1, shapes[1])
    vp = vp.repeat(1, shapes[1])
    ct = torch.zeros((*shapes,), device=device)

    aif3 = torch.full((*shapes,), aif[3], device=device)
    aif5 = torch.full((*shapes,), aif[5], device=device)
    aif7 = torch.full((*shapes,), aif[7], device=device)
    aif8 = torch.full((*shapes,), aif[8], device=device)
