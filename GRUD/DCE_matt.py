...
def fit_tofts_model_pepijn(Ct, t, aif, X0 = (1.0, 0.5, 0.3, 0.03), bounds = ((1e-8, 1e-2, 1e-8, 1e-8), (3.0, 2.0, 1.0, 0.1)),jobs=4, model='Cosine8'):
    '''
    Solves the Extended Tofts model for each voxel and returns model parameters using the computationally efficient AIF as described by Orton et al. 2008 in https://doi.org/10.1088/0031-9155/53/5/005.
    :param Ct: Concentration curve as a N x T matrix, with N the number of curves to fit and T the number of time points
    :param t: Time samples at which Ct is measured
...
def Cosine8AIF_ExtKety_pepijn(t, aif, ke, dt, ve, vp):
    """
    in
    t size: (array: num_time)
    aif size: (dict: aif_num 8)
    ke, dt, ve, vp size: (array: num_samples)
    out
    C size: (num_samples, num_time)
    """
    out_shape = (len(ke), len(t))
    t = np.zeros(out_shape)
    aif_t0 = np.full((*out_shape,), aif['t0'])
    aif_mb = np.full((*out_shape,), aif['mb'])
    aif_mm = np.full((*out_shape,), aif['mm'])
    dt = np.repeat(dt[:, np.newaxis], out_shape[1], axis=1)
    ke = np.repeat(ke[:, np.newaxis], out_shape[1], axis=1)
    ve = np.repeat(ve[:, np.newaxis], out_shape[1], axis=1)
    vp = np.repeat(vp[:, np.newaxis], out_shape[1], axis=1)
    ...
    cpBolus = aif['ab'] * aif['mm'] * ConvBolusExp_pepijn(t, aif_mb, aif_mm)
    input()
    cpRecirc = aif['ab'] * aif['ar'] * CosineBolus(t - aif['tr'], aif['mr'])
    cpWashout = aif['ab'] * aif['ae'] * ConvBolusExp(t - aif['tr'], aif['mr'], aif['me'])
...
def ConvBolusExp_pepijn(t, m, k):
    tB = 2 * pi / m
    t=array(t)
    I1 = ((t > 0).any(axis=0) & (t < tB).any(axis=0))
    I2 = (t <= tB).any(axis=0)
    print(len(I1))
    print(len(I2))
    input()
