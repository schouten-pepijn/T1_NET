...
def GRU_preprocessing(X, fa, mask, length):
    
    shapes = X.size()
    delta = torch.zeros_like(fa)
    last = torch.zeros_like(fa)
    ...
    for i in range(shapes[0]):
        for j in range(shapes[1]): 
            if j == 0:
                ...
                if mask[i, j-1]:
                    delta[i, j] = math.radians(1)
                else:
                    delta[i, j] = math.radians(1) + delta[i, j-1]
                last[i, j] = x_temp
            if mask[i, j]:
                x_temp = X[i, j]
                    
    
    mean = torch.sum(X, dim=1, keepdim=True) / length
    mean = mean.expand(*shapes)
    
    mask = mask.float()
   
    return delta, last, mean, mask
...
def train_T1_transformer(hp, params_T1_true, net=None,
    ...
    for epoch in range(hp.training.T10_epochs):
        ...
        for i, X_idx in enumerate(trainloader):
            ...
            X_delta, X_last, X_mean, X_mask = GRU_preprocessing(X_fa_batch,
                                                                        fa_batch,
                                                                        fa_mask_batch,
                                                                        fa_len_batch)
            ...
            X_mask = X_mask.float().to(hp.device)
            X_mean = X_mean.to(hp.device)
            
      

            optimizer.zero_grad()   

            X_pred, T10_pred, M0_pred, hn_out, X_gru = net(X_fa=X_fa_batch,
                                                           X_last=X_last,
                                                           X_mean=X_mean,
                                                           X_mask=X_mask,
                                                           X_delta=X_delta,
            ...
            if not i % 100:
                ...
                hn_plot = hn_out.cpu().detach().numpy()
                X_gru_plot = X_gru.cpu().detach().numpy()
                X_in_plot = X_fa_batch.cpu().detach().numpy()
                
                np.save('X_gru.npy', X_gru_plot)
                np.save('hn_plot.npy', hn_plot)
                ...
                fig = plt.figure()
                plt.plot(X_gru_plot[0])
                plt.plot(X_in_plot[0], '.')
                plt.savefig('X_gru.png')
                plt.close(fig)
        ...
        with torch.no_grad():
            for i, X_idx in enumerate(tqdm(valloader, position=0, leave=True), 0):
                ...
                X_last = X_last.to(hp.device)
                X_mean = X_mean.to(hp.device)
                X_mask = X_mask.to(hp.device)

                optimizer.zero_grad()   
         
                X_eval, T10_eval, M0_eval, _, _ = net(X_fa=X_fa_batch,
                                                      X_last=X_last,
                                                      X_mean=X_mean,
                                                      X_mask=X_mask,
                                                      X_delta=X_delta,
