...
def predict_T10(hp, net, params):
    ...
    fa_full = torch.FloatTensor([math.radians(i) for i in range(hp.xFA[2][0], hp.xFA[2][1], 1)])
    fa_delta = torch.diff(fa_full)
    delta_first = torch.FloatTensor([0])
    fa_delta = torch.cat((delta_first, fa_delta))
    ...
    with torch.no_grad():
        for i, X_idx in enumerate(tqdm(inferloader, position=0, leave=True), 0):            
            ...
            """
            X_norm = torch.sum(X_fa_batch, dim=1, keepdim=True) / fa_len_batch
            X_norm = X_norm.expand(*X_fa_batch.size())
            
            # X_fa_batch = X_fa_batch / X_norm
            
            X_delta = torch.zeros_like(fa_batch)
            X_delta_bw = torch.zeros_like(fa_batch)
            
            X_last = torch.zeros_like(fa_batch)
            X_next = torch.zeros_like(fa_batch)
            
            for ii, (fa_i, x_i) in enumerate(zip(fa_batch, X_fa_batch)):
                step_fa = math.radians(1)
                last_fa = 0
                last_fa_bw = math.radians(hp.xFA[2][1])
                last_x = x_i[0]
                next_x = x_i[-1]
                for j, (fa_j, x_j) in enumerate(zip(fa_i, x_i)): 
                    X_last[ii, j] = last_x
                    X_next[ii, -(j+1)] = next_x

                    if fa_mask_batch[ii, j]:
                        X_delta[ii, j] = fa_j - last_fa
                        last_fa = fa_j
                        last_x = x_j
                    else:
                        X_delta[ii, j] = step_fa

                    if fa_mask_batch[ii, -(j+1)]:
                        X_delta_bw[ii, -(j+1)] = last_fa_bw - fa_batch[ii, -(j+1)]
                        last_fa_bw = fa_batch[ii, -(j+1)]
                        next_x = X_fa_batch[ii, -(j+1)] 
                    else:
                        X_delta_bw[ii, -(j+1)] = step_fa
            
            X_delta = X_delta.to(hp.device)
            X_delta_bw = X_delta_bw.to(hp.device)
            X_last = X_last.to(hp.device)
            X_next = X_next.to(hp.device)
            X_mask = fa_mask_batch.float().to(hp.device)

            X_mean = torch.sum(X_fa_batch, dim=1, keepdim=True) / fa_len_batch
            X_mean = X_mean.expand(*X_fa_batch.size())

            X_pred, T10_pred, M0_pred = net(X_fa=X_fa_batch,
                                            X_last=X_last,
                                            X_mean=X_mean,
                                            X_mask=X_mask,
                                            X_next=X_next,
                                            X_delta=X_delta,
                                            X_delta_bw=X_delta_bw,
                                            fa_vals=fa_batch,
                                            fa_len=fa_len_batch,
                                            TR_vals=TR_batch)
