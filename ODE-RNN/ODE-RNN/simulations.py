...
def run_simulations(hp, eval=False, var_seq=False):
    ...
    if hp.pretrained:
        ...
        if bool(file_T10):
            ...
            net_T10 = trans.T10_transformer(copy.deepcopy(hp)).to(hp.device)
            net_T10.load_state_dict(
                torch.load(os.path.join('pretrained', file_T10[0]), map_location=hp.device))
            net_T10.to(hp.device)
            net_T10 = train.train_T1_transformer(hp, params_T1_true, net=net_T10)
        else:
...
def predict_T10(hp, net, params):
    ...
    fa_union = torch.FloatTensor(np.unique(params.fa))
    ...
    with torch.no_grad():
        for i, X_idx in enumerate(tqdm(inferloader, position=0, leave=True), 0):            
            ...
            X_pred, T10_pred, M0_pred, _ = net(X_fa_in=X_fa_batch,
                                            fa_vals=fa_batch,
