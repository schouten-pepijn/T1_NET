...
def train_T1_transformer(hp, params_T1_true, net=None,
    ...
    fa_union = torch.FloatTensor(np.unique(params_T1_true.fa))
    fa_union = fa_union[fa_union != 0] / np.pi * 180
    fa_union = fa_union.to(hp.device)
    ...
    for epoch in range(hp.training.T10_epochs):
        ...
        for i, X_idx in enumerate(trainloader):
            ...
            # X_fa_batch /= M0_batch.unsqueeze(-1)

            X_pred, T10_pred, M0_pred, hidden_ode = net(X_fa_in=X_fa_batch,
                                          fa_vals=fa_batch,
            ...
            # mean = torch.sum(X_fa_batch, dim=1) / fa_len_batch.squeeze()
            # batch_loss = X_fa_batch / mean.unsqueeze(dim=1)
            
            batch_loss = X_fa_batch
            ...
            if not i % 10:
                ...
                hidden_ode_np = hidden_ode.cpu().detach().numpy()
                np.save('hidden_state.npy', hidden_ode_np)
                
                plt.figure(dpi=200)
                # for i in hidden_ode:
                plt.plot(hidden_ode_np[0])
                plt.savefig('hidden_state.png', format='png')
                plt.close()
        ...
        with torch.no_grad():
            for i, X_idx in enumerate(tqdm(valloader, position=0, leave=True), 0):
                ...
                X_eval, T10_eval, M0_eval, _ = net(X_fa_in=X_fa_batch,
                                                fa_vals=fa_batch,
                                                fa_mask=fa_mask_batch,
                                                fa_len=fa_len_batch,
                                                TR_vals=TR_batch,
                ...
                # batch_loss = X_fa_batch / X_fa_batch[:, 1].unsqueeze(dim=-1)
                # batch_loss = X_fa_batch / torch.mean(X_fa_batch, dim=1, keepdims=True)
                # mean = torch.sum(X_fa_batch, dim=1) / fa_len_batch.squeeze()
                # batch_loss = X_fa_batch / mean.unsqueeze(dim=1)
                # batch_loss = X_fa_batch / torch.mean(X_fa_batch, dim=1, keepdim=True)
