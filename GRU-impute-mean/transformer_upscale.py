...
class T10_transformer(nn.Module):
    ...
    def __init__(self, hp):
        ...
        self.pe = SineActivation(d_model, d_model)
        # self.layers = get_clones(EncoderLayer(d_model, heads), N_layers)
        self.norm_1 = Norm(d_model)
        self.norm_2 = Norm(d_model)
        
        self.score = nn.Sequential(nn.Linear(d_hidden, 1), nn.Softmax(dim=1))
        
        # debug
        # self.att = MultiHeadAttention(heads, d_model)
        # self.fcn = FeedForward(d_model, 2048)
        self.enc_block_1 = EncoderLayer(d_model, d_hidden, 3)
        # self.enc_block_2 = EncoderLayer(d_hidden, d_hidden, 3)
        ...
        self.regressor = nn.Sequential(
                                       nn.Linear(seq_len*d_hidden, seq_len*d_hidden//2),
                                       nn.GELU(),
                                       nn.Linear(seq_len*d_hidden//2, 1)
                                       )
    ...
    def forward(self, X_fa, fa_vals, fa_len, fa_mask):
        ...
        # X = X[:, :6].contiguous().unsqueeze(dim=2)
        # FA_in = FA_in[:, :6].clone().unsqueeze(dim=2)
        
    # )    print(torch.max(X))
    #     print(torch.max(FA_in))
    #     input(
        
        
        X = X_fa.unsqueeze(dim=2)
        X = X.expand(shapes_fa[0], shapes_fa[1], shapes_fa[1])
        
        X = self.pe(X)
    
        mask = ~fa_mask.type(torch.bool).to(self.hp.device).unsqueeze(dim=2)

        X = self.enc_block_1(X, mask=mask)
        
        # X = self.norm_2(X)
        
        # X = self.enc_block_2(X, mask=mask)
        
        
        score = self.score(X)
        
        hidden_X = torch.sum(X*score, dim=1)
        ...
        # X = X.view(shapes_fa[0], -1)
        
        T10 = self.regressor(hidden_X)
