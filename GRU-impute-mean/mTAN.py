...
class T10_transformer(nn.Module):
    def __init__(self, hp):
        ...
        # self.torch_att = nn.MultiheadAttention(embed_dim=1,
                                               # num_heads=1,
                                               # kdim=1,
                                               # vdim=1,
                                               # batch_first=True)
    ...
    def forward(self, X_fa_in, fa_vals, fa_mask, fa_len, TR_vals):
        ...
        X_fa = torch.where(X_fa_in == torch.tensor(0.),
                           torch.tensor(1.).to(self.device), X_fa_in)

        X_fa = torch.cat((X_fa.unsqueeze(dim=-1),
                          fa_vals.unsqueeze(dim=-1)), dim=-1)
        
        out, _ = self.rnn(X_fa)

        # out, _ = self.rnn_last(out*self.score_rnn(out))
